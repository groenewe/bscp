...
# or the Py2 client (skips 7 py3-only tests when `python` is Python 2):
...
plus `python -V`), seven tests are skipped by default and reported as
...
functions it probes), and the four `--verify` tests (the convenience b3sum
